...
| [SMT Doctrine](smt-doctrine.md) | **How to model relabeled-identical hardware** ("Stupid Manufacturer Tricks"). One definition carries an **open array of identities** — a relabel is an *entry*, not a description; the "these are the same device" claim is **earned on verified behaviour, never assumed from resemblance**; the cosmetic-vs-behavioural dividing line (*would driving code have to change?*); why a vendor id is not a manufacturer-tier constant and branding ≠ reported identity; **relabel-robust recognition** (exact-id fast path, fingerprint fallback, unknown only when both fail). The device-model tree handles real *variation*; SMT handles *sameness wearing a disguise*. |
