...
| [Hardware Compatibility Doctrine](hardware-compatibility-doctrine.md) | **How a product that touches more than one device tracks what each can actually do — a living Hardware Compatibility List (HCL).** The moment there's a second device, three questions recur forever: *have we seen this one? what can it do? did we verify, or hope?* The HCL makes each a **committed fact with a provenance tag**, so "we assumed read-write" can never masquerade as "we verified read-write." Every device is a row: **device** (a real model, never an invented placeholder — the SMT "FS-300B" trap), **access class** (`RW`/`RO`/`WO`/`Actions`, per-capability where mixed — write-only means "sent" is never truth), **verified-state** (`[V]` on real hardware / `[D]` documented / `[A]` assumed — **`[A]` is the default until driven**, and a hoped access class reads `[A]`, never a bare `RW`), transport/protocol, notes, dated+unit-specific provenance. It is **not** the device descriptor (that's what a device *declares*); it is the **verification ledger about tested reality** — on disagreement the HCL's tested reality wins. **SSoT split:** the rule is family, the device rows live at each product (like Network Allocation). Updated in the same change as the verification; a stale `[V]` (a firmware ago) is visible as stale. Composes Device-Model (descriptor), Confidence & Provenance (tags), SMT (sameness earned), Verification (how a row earns `[V]`); restates none. |
...
| [Network Allocation Doctrine](network-allocation-doctrine.md) | **How the family hands out collision-prone network namespaces — FOSS projects only.** Ports, mDNS service names, private IPv4 networks, and BLE-mesh addresses are shared spaces where two projects can silently claim the same slot; a living registry makes the collision *inexpressible*. **Ports:** main `2900–2939` (290X owner-reserved, rest sequential, wrap to `2840–2899`); a reservation is **TCP+UDP together**; prefer one port — second channel resolves multiplex → **ephemeral+mDNS** (the debug interface, Android-style) → fixed `3129–3224` (no wrap) only if it must be well-known; OS-ephemeral is IANA `49152–65535`. **mDNS:** advertise as `_<slug>._tcp`, aux as `<slug>-<role>`; never touch a foreign hosts file (Conventions owns the mechanics). **Networks:** all family-defined nets in `172.23.0.0/16` — containers `/18` (a `/24` per project), examples `172.23.240.0/20`, rest reserved; VMs get **no block** (bridged/NAT, or a product "static-or-DHCP, default DHCP" rule). **BLE-mesh:** unicast sequential from `0x0001` (reclaimable), groups from `0xC001` (`0xC000` = all-in-mesh), virtual/fixed reserved. The registry is the source of truth — an assignment is real when committed, not when a service binds it. Companion to Service Foundations + WS Control; the "family-coordinated port contract" they reference. |
| [Secrets & Credentials Doctrine](secrets-credentials-doctrine.md) | **How the family handles a secret — cross-project.** A secret is **referenced, never embodied**: code names *which* secret, the value is fetched from a store at runtime and lives in exactly one home — never source, git, a log, or a cleartext data file. Resolves through the **same config chain as every setting** (CLI/env → store), with two secret-specific deviations: **no compiled default** (a hardcoded secret is the catastrophe), and the store tier is **keystore-preferred / mode-0600-file-sanctioned** — a *setting* with a safe default (Decision §4), because the headless Pi and FOSS user make the file first-class, not a fallback. A missing required secret is **loud and fatal with a map** (name the secret, the chain searched, the fix) — the LiteController "TEA key unavailable" lesson made correct. Never-in-git is **structural**: gitignored key paths + a pre-commit path-block gate **verified by attack**, and the data file names the secret, never holds it. Off-box ⇒ credential coupled (ws-control §3). Consolidates the global secret rule + Service-Foundations + WS-Control; restates none. |
| [Logging & Diagnostics Doctrine](logging-diagnostics-doctrine.md) | **How anything the family builds emits its log — cross-project.** The log is a **first-class observation channel** (often the only place you see what the hardware *actually* did) that must be three things at once: **human-readable, never parsed as an API, and safe.** Lifts Service-Foundations §6 (loosely-parsed stream, text-never-an-API, may double as a published WS channel); adds the two pieces no existing home covered — the **level set** (error/warn/info/debug/trace, the *ecosystem-standard* set adopted per Conventions §1, not invented; active level settable through the config chain, default `info`) and **what must never be logged**: a secret never appears in any line or stream, at **any** level including `trace` (where the temptation is highest), made **structural** by a redaction boundary at the source + masking raw `trace` frame dumps at the dump site, **verified by attack** — not a scan after the fact. Restates the global "no unguarded debug output in production" as its logging face. The one exception to the secret floor (a root, per-secret, audited reveal) is owned by the **Debugging Doctrine**, referenced not restated. A log may be shipped to a **remote sink** — standard protocol first (syslog/OTLP to any aggregator), off by default, TCP+TLS-default / UDP-optional, **buffer-then-drop, never block** (local log stays complete), §4 redaction verified on the remote path. Composes Secrets §0; points at Conventions for per-OS location. |
| [Debugging Doctrine](debugging-doctrine.md) | **The break-glass, root-privileged diagnostic capability a family product owes — broad strokes; the shape is the *debug-channel blueprint*.** Debug is **root** (it can do anything that can be done) and *because* it is that powerful it is **off by default, absent until enabled**, every step toward off-box a separate conscious act. A **superset of authority**, never a hiding place — **nothing a normal client needs may live only in debug** (no front-door hole). Explicit privilege is **not** the "privileged back channel" WS-Control §0 forbids: §0's target is *concealment*, and this is an advertised, gated, auditable surface (`sudo` alongside a user account). **Two dependent gates** (Android USB→Wireless model): local debug (loopback) then network debug (dependent), the latter coupling a **distinct settable credential** + off-box bind + a **most-restrictive host/network allowlist of the operator's own addresses** — a valid credential *and* an allowed source, both required. Owns the **one carve-out** to the logging §4 secret-redaction floor: a **per-secret, explicit, audited** reveal (reveal-not-enable applied to secrets — hidden not absent, the reveal itself logged). Composes WS-Control, Logging, Decision, Service-Foundations; defers attach/capability-set/contract/conformance to the blueprint. |
| _Product Versioning & Release Doctrine_ (planned) | **PLACEHOLDER — not yet written.** How a *product* (not an interface or a doc) versions and ships: semver policy, bump-the-whole-tree on release, tag/release flow, changelog, coupled artifacts (e.g. Homebrew tap bump). Distinct from `interface-stability` (interface versioning) and `spec-promotion` (doc versioning). |
| _Testing Strategy Doctrine_ (planned) | **PLACEHOLDER — not yet written.** The *shape* of the test suite (distinct from `verification`/`cold-start-acceptance`, which govern *proving done*): unit/integration/GUI layers that must disagree when broken; "a reported bug means a defective test"; headless-CI patterns; level tiers. |
